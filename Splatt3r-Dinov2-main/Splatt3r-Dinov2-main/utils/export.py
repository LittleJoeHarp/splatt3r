...
def log_batch_files(batch, color, depth, mask, view1, view2, pred1, pred2, save_dir, should_save_3d=False):
    ...
    # Handle both ScanNet++ (has depthmap/valid_mask) and CO3D (doesn't have them)
    ...
    for b in range(min(context_images.shape[0], 4)):
        ...
        # Only save depthmap and valid_mask if they exist (ScanNet++)
        if "depthmap" in batch["context"][0]:
        ...
        if "valid_mask" in batch["context"][0]:
    ...
    for b in range(min(target_original_images.shape[0], 4)):
        ...
        # Only save depthmap if it exists (ScanNet++)
        if "depthmap" in batch["target"][0]:
