...
def calculate_loss_mask(batch):
    ...
    # Check if depth maps are available (ScanNet++) or not (CO3D)
    if 'depthmap' not in batch['target'][0]:
        # CO3D dataset doesn't have depthmaps
        # Return a mask of all ones (no masking)
        target_view = batch['target'][0]
        if 'original_img' in target_view:
            h, w = target_view['original_img'].shape[-2:]
        else:
            h, w = target_view['img'].shape[-2:]
        
        # Return shape (B, 1, H, W) for CO3D (batch_size=1, single target view)
        mask = torch.ones(1, 1, h, w, dtype=torch.bool, device=target_view['img'].device)
        return mask
